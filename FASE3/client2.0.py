...
def main():
    ...
    # Serializzare la chiave pubblica in X962 (formato corretto per ECDHE)
    public_key_bytes = public_key.public_bytes(
        encoding=serialization.Encoding.X962,  # Serializzazione in X962
        format=serialization.PublicFormat.UncompressedPoint
    )

    # Creare pacchetto iniziale con la chiave pubblica
    initial_packet = create_initial_packet(1, 1, public_key_bytes)
